...
impl TaskControlBlock {
    ...
    pub fn spawn(self: &Arc<TaskControlBlock>, path: &str) -> Option<Arc<TaskControlBlock>> {
        let mut parent_inner = self.inner_exclusive_access();
        let (memory_set, user_sp, entry_point) = MemorySet::from_elf(get_app_data_by_name(path)?);
        let trap_cx_ppn = memory_set
            .translate(VirtAddr::from(TRAP_CONTEXT).into())
            .unwrap()
            .ppn();
